...
const nextQuestion = id => {
  Array
    ...
    .forEach(e => e.setAttribute('data-active', 'false'));

  document.getElementById(`q-${id}`).setAttribute('data-active', 'true');

  setTimeout(() => {
    if (document.querySelector(`#q-${id} input[type="text"]`)) {
      document.querySelector(`#q-${id} input[type="text"]`).focus();
    }
    if (document.querySelector(`#q-${id} .button`)) {
      document.querySelector(`#q-${id} .button`).focus();
    }
  }, 10);
};
...
Array
  ...
  .forEach(
    e => e.addEventListener('click', ev => {
      Array
        .from(document.getElementsByClassName('error'))
